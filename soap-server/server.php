...
{	private $host="localhost";	
	...
	*/
	...
	public function tambah_mahasiswa($nim_mhs,$nama_mhs,$alamat_mhs,$no_telp_mhs)
	{	// menggunakan fungsi dalam satu class
		$nim_mhs = $this->filter($nim_mhs);
		
		// query anti sql injection
		$query = $this->conn->prepare("INSERT INTO mahasiswa (nim,nama,alamat,no_telp) VALUES (?,?,?,?)");		
		$query->execute(array($nim_mhs,$nama_mhs,$alamat_mhs,$no_telp_mhs));	
		
		// menghapus query dari memory 
		$query->closeCursor();	
		// $query = null;	
		// menghapus variabel dari memory
		unset($nim_mhs,$nama_mhs,$alamat_mhs,$no_telp_mhs);	
	}
	
	public function daftar_mahasiswa()
	{	// query 
		$query = $this->conn->prepare("select nim, nama, alamat, no_telp from mahasiswa order by nim");
		$query->execute();
	...
	{	// menggunakan fungsi dalam satu class
		...
		// query 
		$query = $this->conn->prepare("select nim, nama, alamat, no_telp from mahasiswa where nim = ? ");
	...
	public function ubah_mahasiswa($nim_mhs,$nama_mhs,$alamat_mhs,$no_telp_mhs)
	{	// menggunakan fungsi dalam satu class
		$nim_mhs = $this->filter($nim_mhs);
	
		// query 
		$query = $this->conn->prepare("UPDATE mahasiswa SET nama = ?, alamat = ?, no_telp = ? WHERE nim = ?");
		$query->execute(array($nama_mhs,$alamat_mhs,$no_telp_mhs,$nim_mhs));
...
$options=array('uri'=>'http://127.0.0.1:8080');
